...
def main():
    ...
    result = tu.run(
        {
            ...
            "arguments": {"query": "cancer", "page_size": 5},
        }
    )
    ...
    result = tu.run(
        {
            "name": "PRIDE_search_proteomics",
            "arguments": {"query": "brain", "page_size": 4},
        }
    )
    ...
    result = tu.run(
        {
            "name": "PRIDE_search_proteomics",
            "arguments": {"query": "human", "page_size": 4},
        }
    )
    ...
    if result and result.get("status") == "success":
        ...
        for i, exp in enumerate(data[:2], 1):
            ...
            accession = exp.get("accession", "Unknown")
            species = (
                exp.get("species", ["Unknown"])[0] if exp.get("species") else "Unknown"
            )
