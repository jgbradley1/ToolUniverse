...
def example_lookup_gene():
    ...
    if result.get("status") == "success":
        ...
        print(
            f"Location: {data.get('seq_region_name')}:{data.get('start')}-{data.get('end')}"
        )
    else:
    ...
    result = tu.run(
        {
            ...
            "arguments": {"sequence_id": "ENSG00000139618", "type": "genomic"},
        }
    )
...
def example_get_protein_sequence():
    ...
    result = tu.run(
        {
            "name": "ensembl_get_sequence",
            "arguments": {"sequence_id": "ENSG00000139618", "type": "protein"},
        }
    )
...
def example_get_variants():
    ...
    if result.get("status") == "success":
        ...
        for i, variant in enumerate(data[:5]):  # Show first 5
            print(
                f"  {i + 1}. {variant.get('id')}: {variant.get('allele_string')} ({variant.get('consequence_type')})"
            )
    else:
        print(f"Error: {result.get('error')}")
