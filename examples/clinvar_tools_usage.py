    ...
    result = tu.run(
        {
            ...
            "arguments": {"gene": "BRCA1", "max_results": 10},
        }
    )
...
def example_search_variants_by_condition():
    ...
    result = tu.run(
        {
            "name": "clinvar_search_variants",
            "arguments": {"condition": "breast cancer", "max_results": 5},
        }
    )
...
def example_get_clinical_significance():
    ...
    if result.get("status") == "success":
        if "formatted_data" in result:
            ...
            print(
                f"  Description: {formatted['germline_classification']['description']}"
            )
            print(
                f"  Review Status: {formatted['germline_classification']['review_status']}"
            )
            print(
                f"  Last Evaluated: {formatted['germline_classification']['last_evaluated']}"
            )
            print(
                f"  FDA Recognized: {formatted['germline_classification']['fda_recognized']}"
            )
...
def example_combined_search():
    ...
    search_result = tu.run(
        {
            "name": "clinvar_search_variants",
            "arguments": {"gene": "BRCA2", "max_results": 3},
        }
    )
