...
class TransportClosedDebugger:
    ...
    def test_tools_direct(self) -> Dict[str, Any]:
        ...
        try:
            ...
            tooluni.load_tools()
            self.log(
                f"✅ ToolUniverse 初始化成功，加载了 {len(tooluni.all_tool_dict)} 个工具"
            )
                    ...
                    if isinstance(result, dict) and "error" in result:
                        ...
                        if self.verbose:
                            ...
                            if isinstance(result, dict) and len(str(result)) < 1000:
                                self.log(
                                    f"结果预览: {json.dumps(result, indent=2)[:500]}..."
                                )
    ...
    def print_summary(self, all_results: List[Dict[str, Any]]):
        ...
        if direct_result and stdio_result:
            ...
            for i, direct_test in enumerate(direct_result["tests"]):
                stdio_test = (
                    stdio_result["tests"][i] if i < len(stdio_result["tests"]) else None
                )

                if direct_test["name"] == stdio_test["name"]:
                    self.log(f"\n工具: {direct_test['name']}")
                    self.log(
                        f"  直接测试: {direct_test['execution_time']:.2f}s, 成功: {direct_test['success']}"
                    )

                    if stdio_test:
                        self.log(
                            f"  stdio测试: {stdio_test['execution_time']:.2f}s, 成功: {stdio_test['success']}"
                        )
                        if stdio_test.get("transport_closed"):
                            ...
                            if direct_test["timeout"]:
                                self.log(
                                    f"  💡 原因分析: 工具本身超时（{direct_test['execution_time']:.2f}s）"
                                )
                            elif direct_test["execution_time"] > 30:
                                self.log(
                                    f"  💡 原因分析: 工具执行时间过长（{direct_test['execution_time']:.2f}s）"
                                )
                            else:
        ...
        if stdio_result and stdio_result["summary"]["transport_closed"] > 0:
            ...
            # 检查 GraphQL 工具超时
            opentargets_test = next(
                (t for t in direct_result["tests"] if "OpenTargets" in t["name"]), None
            )
            if opentargets_test and opentargets_test["timeout"]:
                self.log("1. GraphQL 工具缺少 timeout 参数")
                self.log(
                    "   - 在 src/tooluniverse/graphql_tool.py 的 execute_query() 中添加 timeout=60"
                )
