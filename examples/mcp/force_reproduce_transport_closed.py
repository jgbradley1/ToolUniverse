...
class TransportClosedReproducer:
    ...
    async def test_slow_queries(self):
        ...
        slow_queries = [
            ...
            },
            {
                "name": "UniProt_search",
                "args": {
                    "query": "very_specific_and_long_query_that_might_take_time",
                    "limit": 1000,
                },
                "description": "大量结果的查询",
            },
                ...
                async with ClientSession(read, write) as session:
                    ...
                    for i, query in enumerate(slow_queries):
                        ...
                        try:
                            ...
                            execution_time = time.time() - start_time
                            self.log(
                                f"⏰ 查询超时（5秒），实际耗时 {execution_time:.2f}秒"
                            )
            ...
            async with stdio_client(server) as (read, write):
                async with ClientSession(read, write) as session:
                    ...
                    try:
                        ...
                        execution_time = time.time() - start_time
                        self.log(
                            f"⏰ 并发调用超时（30秒），实际耗时 {execution_time:.2f}秒"
                        )
    ...
    async def test_resource_stress(self):
        ...
        try:
            ...
            resource.setrlimit(
                resource.RLIMIT_AS, (100 * 1024 * 1024, original_limit[1])
            )  # 100MB
                ...
                async with ClientSession(read, write) as session:
                    ...
                    for i in range(10):
                        ...
                        try:
                            ...
                            execution_time = time.time() - start_time
                            self.log(
                                f"⏰ 调用超时（10秒），实际耗时 {execution_time:.2f}秒"
                            )
                    ...
                    async with ClientSession(read, write) as session:
                        ...
                        for query in problematic_queries:
                            ...
                            try:
                                ...
                                execution_time = time.time() - start_time
                                self.log(
                                    f"⏰ 调用超时（2秒），实际耗时 {execution_time:.2f}秒"
                                )
