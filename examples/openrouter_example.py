...
Provide a structured analysis.""",
    ...
    result = tu.execute_tool("Claude_Analyzer", {"concept": "CRISPR gene editing"})
    ...
    if isinstance(result, str):
        try:
            ...
            for i, tool in enumerate(parsed.get("tools", []), 1):
                ...
                print(
                    f"   Description: {tool.get('description', 'No description')[:100]}..."
                )
        except:
            print(result)
    else:
...
def example_6_model_comparison():
    ...
    for model_id, model_name in models:
        ...
        try:
            ...
            print(
                result if isinstance(result, str) else result.get("result", str(result))
            )
...
def example_7_fallback_configuration():
    ...
    tool_config = {
        ...
        "parameter": {
            "type": "object",
            "properties": {"statement": {"type": "string", "required": True}},
            "required": ["statement"],
        },
        "configs": {
            "api_type": "OPENROUTER",
