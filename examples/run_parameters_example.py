...
class CacheAwareTool(BaseTool):
    ...
    def run(self, arguments=None, use_cache=False, **kwargs):
        ...
        return {"query": query, "result": result, "used_cache": use_cache}
...
class ValidationAwareTool(BaseTool):
    ...
    def run(self, arguments=None, validate=True, **kwargs):
        ...
        if not validate:
            ...
            if not data:
                return {"error": "Data cannot be empty", "validation_performed": False}
...
result1 = tu.run_one_function(
    {"name": "cache_aware_search", "arguments": {"query": "python"}}, use_cache=False
)

result2 = tu.run_one_function(
    {"name": "cache_aware_search", "arguments": {"query": "python"}}, use_cache=True
)

# Example 2: Validation-aware tool usage
result3 = tu.run_one_function(
    {"name": "validation_aware_processor", "arguments": {"data": "hello"}},
    validate=True,
)

result4 = tu.run_one_function(
    {"name": "validation_aware_processor", "arguments": {"data": "world"}},
    validate=False,
)
...
result5 = tu.run_one_function(
    {"name": "streaming_reporter", "arguments": {"task": "analysis"}},
    stream_callback=None,
)

result6 = tu.run_one_function(
    {"name": "streaming_reporter", "arguments": {"task": "processing"}},
    stream_callback=my_callback,
)

# Example 4: Comprehensive tool with all parameters
result7 = tu.run_one_function(
    {"name": "comprehensive_tool", "arguments": {"operation": "full_analysis"}},
    stream_callback=my_callback,
    use_cache=True,
    validate=True,
)
...
result8 = tu.tools.cache_aware_search(query="dynamic call", use_cache=True)
