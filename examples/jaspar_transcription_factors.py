...
def main():
    ...
    result = tu.run(
        {
            ...
            "arguments": {"collection": "CORE", "limit": 5},
        }
    )
    ...
    result = tu.run(
        {
            "name": "JASPAR_get_transcription_factors",
            "arguments": {"collection": "CORE", "limit": 8},
        }
    )
    ...
    if result and result.get("status") == "success":
        ...
        for i, tf in enumerate(results[:4], 1):
            ...
            tf_id = tf.get("matrix_id", "Unknown")
            species = (
                tf.get("species", ["Unknown"])[0] if tf.get("species") else "Unknown"
            )
    ...
    result = tu.run(
        {
            "name": "JASPAR_get_transcription_factors",
            "arguments": {"collection": "CORE", "limit": 6},
        }
    )
