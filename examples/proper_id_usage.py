...
def demonstrate_id_conversion():
    ...
    if isinstance(result, dict):
        ...
        if entries:
            uniprot_id = entries[0].get("primaryAccession") or entries[0].get(
                "accession"
            )
    ...
    result = tu.run(
        {
            ...
            "arguments": {"sequence_id": gene_id, "type": "genomic"},
        }
    )
    ...
    result = tu.run(
        {
            ...
            "arguments": {"from_db": "Ensembl", "to_db": "UniProtKB", "ids": gene_id},
        }
    )
