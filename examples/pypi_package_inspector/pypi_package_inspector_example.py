...
def compare_packages():
            ...
            results.append(
                {
                    ...
                    "downloads": result["download_stats"].get(
    ...
    print(f"\n📊 Package Comparison:")
    print(
        f"{'Rank':<6} {'Package':<12} {'Overall':<10} {'Popularity':<12} "
    ...
    for i, pkg in enumerate(results, 1):
        print(
            f"{i:<6} {pkg['name']:<12} {pkg['overall']:<10} "
...
def evaluate_for_dependency():
    ...
    result = tu.run(
        {
            "name": "PyPIPackageInspector",
            "arguments": {"package_name": package_to_evaluate},
        }
    )
