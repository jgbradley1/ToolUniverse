...
def main():
    ...
    result = tu.run(
        {"name": "InterPro_get_protein_domains", "arguments": {"protein_id": "P04637"}}
    )
    ...
    if result and result.get("status") == "success":
        ...
        for domain in data[:3]:
            ...
            print(
                f"     Source: {domain.get('metadata', {}).get('source_database', 'Unknown')}"
            )
    else:
    ...
    result = tu.run(
        {"name": "InterPro_get_protein_domains", "arguments": {"protein_id": "P38398"}}
    )
    ...
    result = tu.run(
        {"name": "InterPro_get_protein_domains", "arguments": {"protein_id": "P01308"}}
    )
