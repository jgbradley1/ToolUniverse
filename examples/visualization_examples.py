...
def save_visualization(result: Dict[str, Any], filename: str):
    ...
    if result.get("success") and "html" in result.get("visualization", {}):
        ...
        print(
            f"❌ Failed to save visualization: {result.get('error', 'Unknown error')}"
        )
...
def example_1_protein_structure_3d():
    ...
    if result["success"]:
        ...
        print(
            f"📏 Dimensions: {result['visualization']['metadata']['width']} × {result['visualization']['metadata']['height']}"
        )
...
def example_2_molecule_2d():
    ...
    for smiles, name in molecules:
        ...
        if result["success"]:
            ...
            save_visualization(
                result, f"molecule_2d_{name.lower().replace(' ', '_')}.html"
            )
        else:
    ...
    for name in molecule_names:
        ...
        result = tu.run(
            {
                "name": "visualize_molecule_2d",
                "arguments": {"molecule_name": name, "width": 500, "height": 500},
            }
        )
...
def example_3_molecule_3d():
    ...
    for smiles, name in molecules_3d:
        ...
        if result["success"]:
            ...
            save_visualization(
                result, f"molecule_3d_{name.lower().replace(' ', '_')}.html"
            )
        else:
            print(f"   ❌ Failed to visualize {name}: {result['error']}")
    ...
    if result["success"]:
        ...
        print(
            f"📊 Generated {result['visualization']['data']['conformer_count']} conformers"
        )
...
def example_4_integration_with_other_tools():
    ...
    try:
        ...
        uniprot_result = tu.run(
            {"name": "uniprot_search", "arguments": {"query": "hemoglobin", "limit": 1}}
        )
        ...
        if uniprot_result.get("success") and uniprot_result.get("results"):
            ...
            print(
                f"   Found protein: {protein_data.get('protein_name')} ({protein_id})"
            )
    ...
    try:
        ...
        chembl_result = tu.run(
            {"name": "chembl_search", "arguments": {"query": "aspirin", "limit": 1}}
        )
...
def example_5_batch_processing():
    ...
    print(
        f"   📊 Success rate: {successful_visualizations / (successful_visualizations + failed_visualizations) * 100:.1f}%"
    )
...
def example_6_error_handling():
    ...
    error_cases = [
        ...
        {
            "name": "visualize_molecule_3d",
            "arguments": {"smiles": "CCO", "style": "invalid_style"},
        },
    ]
    ...
    for i, case in enumerate(error_cases, 1):
        ...
        if result["success"]:
            print(
                f"   ✅ Handled gracefully: {result.get('visualization', {}).get('type', 'unknown')}"
            )
        else:
            print(f"   ❌ Error: {result['error']}")
...
def main():
    ...
    try:
        ...
        print(
            "   All visualization HTML files are saved in the 'visualizations/' directory"
        )
        print(
            "   You can open them in any web browser to view the interactive visualizations"
        )
        print()
        print("💡 Tips:")
