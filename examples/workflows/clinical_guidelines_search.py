...
def is_valid_guideline(guideline: dict) -> bool:
    ...
    keyword_hits = sum(
        ...
        if any(keyword in section for keyword in guideline_keywords)
    )
...
def enhance_search_query(query: str) -> str:
    ...
    enhanced_query = (
        f"{query} AND (guideline OR guidelines OR recommendation OR "
    ...
    if "diabetes" in query.lower():
        enhanced_query += (
            " AND (type 1 diabetes OR type 2 diabetes OR diabetes mellitus)"
        )
    elif "hypertension" in query.lower() or "blood pressure" in query.lower():
        enhanced_query += (
            " AND (hypertension OR high blood pressure OR arterial hypertension)"
        )
    elif "cancer" in query.lower() or "oncology" in query.lower():
        enhanced_query += " AND (cancer OR neoplasm OR oncology OR tumor OR tumour)"
...
def clinical_guidelines_search_workflow(query: str, max_results_per_source: int = 5):
    ...
    sources = [
        ...
        ("TRIP", TRIP_Database_Guidelines_Search, "Evidence-Based Medicine Database"),
    ...
    for source_name, search_function, description in sources:
        ...
        try:
            ...
            search_query = (
                enhanced_query
                if source_name in ["PubMed", "EuropePMC", "OpenAlex", "TRIP"]
                else query
            )
            ...
            if source_name == "PubMed":
                ...
                guidelines = search_function(
                    query=search_query, limit=max_results_per_source
                )
            ...
            if guidelines:
                ...
                valid_guidelines = [g for g in guideline_list if is_valid_guideline(g)]
                ...
                print(
                    f"     Found {len(guideline_list)} results, "
    ...
    results["summary_stats"] = {
        ...
        "avg_citations": sum(g["cited_by_count"] for g in all_guidelines)
        / len(all_guidelines)
        if all_guidelines
        else 0,
    }
...
if __name__ == "__main__":
    ...
    results = clinical_guidelines_search_workflow(
        query="diabetes management", max_results_per_source=3
    )
    ...
    total_found = sum(
        source.get("total_found", 0) for source in results["sources"].values()
    )
    total_filtered = sum(
        source.get("filtered_out", 0) for source in results["sources"].values()
    )
    ...
    for i, guideline in enumerate(results["top_guidelines"][:3], 1):
        print(f"{i}. {guideline['title']} ({guideline['source']}, {guideline['year']})")
        snippet = (
