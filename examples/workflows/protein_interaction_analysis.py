...
def protein_interaction_analysis_workflow(
    gene_list: list, tissue: str = "breast", interaction_type: str = "co-expression"
):
    """
        ...
        if ppi_analysis:
            ...
            if isinstance(ppi_analysis, dict):
                ...
                results["network_stats"] = {
                    ...
                    "input_genes_in_network": len(
                        [n for n in nodes if n in ensembl_ids]
                    ),
                }
                print(f"     Network nodes: {len(nodes)}")
                print(f"     Network edges: {len(edges)}")
                print(
                    f"     Input genes in network: {results['network_stats']['input_genes_in_network']}"
                )
            else:
    ...
    gene_symbols = [
        annotation["gene_name"] for annotation in results["gene_annotations"].values()
    ]
