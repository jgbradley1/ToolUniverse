...
def unified_drug_discovery_workflow(
    ...
    try:
        ...
        try:
            ...
            if disease_info and isinstance(disease_info, dict):
                ...
                if disease_info.get("success") and disease_info.get("result"):
                    ...
                    print(
                        f"   ✅ Disease information retrieved: {disease_info['result'].get('name', 'N/A')}"
                    )
                    ...
                    if disease_id:
                        ...
                        if (
                            targets_info
                            and isinstance(targets_info, dict)
                            and targets_info.get("success")
                        ):
                            results["disease_analysis"]["associated_targets"] = (
                                targets_info["result"]
                            )
                            print(
                                f"   ✅ Found {len(targets_info['result'])} associated targets"
                            )
                            ...
                            if not target_genes:
                                target_genes = [
                                    ...
                                    for target in targets_info["result"][:10]
                                ]
                                print(
                                    f"   🎯 Selected top targets: {', '.join(target_genes[:5])}..."
                                )

                # Check for direct hits format (GraphQL response)
                elif (
                    disease_info.get("data")
                    and disease_info["data"].get("search")
                    and disease_info["data"]["search"].get("hits")
                ):
                    hits = disease_info["data"]["search"]["hits"]
                    if hits and len(hits) > 0:
                        results["disease_analysis"]["disease_info"] = hits[0]
                        print(
                            f"   ✅ Disease information retrieved: {hits[0].get('name', 'N/A')}"
                        )
                        ...
                        if disease_id:
                            ...
                            if targets_info and isinstance(targets_info, dict):
                                if (
                                    targets_info.get("data")
                                    and targets_info["data"].get("disease")
                                    and targets_info["data"]["disease"].get(
                                        "associatedTargets"
                                    )
                                ):
                                    associated_targets = targets_info["data"][
                                        "disease"
                                    ]["associatedTargets"]
                                    ...
                                    print(
                                        f"   ✅ Found {associated_targets.get('count', 0)} associated targets"
                                    )

                                    # Extract target genes if not provided
                                    if not target_genes and associated_targets.get(
                                        "rows"
                                    ):
                                        target_genes = [
                                            row["target"]["approvedSymbol"]
                                            for row in associated_targets["rows"][:10]
                                        ]
                                        print(
                                            f"   🎯 Selected top targets: {', '.join(target_genes[:5])}..."
                                        )
                                elif targets_info.get("success") and targets_info.get(
                                    "result"
                                ):
                                    ...
                                    print(
                                        f"   ⚠️ Unexpected targets response format: {targets_info}"
                                    )
                ...
                # Check for direct list format
                    ...
                    print(
                        f"   ✅ Disease information retrieved: {disease_info[0].get('name', 'N/A')}"
                    )
                    ...
                    print(
                        f"   ⚠️ Disease analysis returned unexpected format: {disease_info}"
                    )
                    # No fallback targets - rely on user input or LLM analysis
                    if not target_genes:
                        print(
                            f"   ⚠️ No target genes provided and disease analysis failed"
                        )
            else:
                print(
                    f"   ⚠️ Disease analysis returned unexpected format: {type(disease_info)}"
                )
                # No fallback targets - rely on user input or LLM analysis
                if not target_genes:
        ...
        if enable_llm_analysis and target_genes:
            try:
                ...
                if disease_analysis and disease_analysis.get("success"):
                    results["llm_insights"]["disease_analysis"] = disease_analysis[
                        "result"
                    ]
                    print(f"   🧠 LLM disease analysis completed")
                else:
                    print(
                        f"   ⚠️ LLM disease analysis failed: {disease_analysis.get('error', 'Unknown error')}"
                    )
            except Exception as e:
            ...
            for target in target_genes[:5]:  # Limit to top 5 targets
                try:
                    ...
                    if target_drugs and target_drugs.get("success"):
                            ...
                            validated_targets.append(
                                {
                                    ...
                                    "sample_drugs": [
                                        drug.get("drug_name") for drug in drugs[:3]
                                    ],
                                }
                            )
                        ...
                        validated_targets.append(
                            {"target_name": target, "druggable": True, "drug_count": 0}
                        )
                ...
                try:
                    ...
                    if target_drugs and target_drugs.get("success"):
                        for drug in target_drugs["result"][:3]:  # Top 3 per target
                            compound_name = (
                                drug.get("pref_name")
                                or drug.get("molecule_chembl_id")
                                or "Unknown"
                            )
                    ...
                    print(
                        f"   ❌ Target-based screening failed for {target['target_name']}: {str(e)}"
                    )
        ...
        if use_advanced_strategies:
            ...
            for mechanism in mechanisms:
                try:
                    ...
                    print(
                        f"   ❌ Mechanism-based screening failed for {mechanism}: {str(e)}"
                    )

        # Strategy 3: Indication-based screening (fallback)
        if len(candidate_compounds) < 5:
            try:
                indication_drugs = FDA_get_drug_names_by_indication(
                    disease_name, limit=5
                )
                if indication_drugs and isinstance(indication_drugs, list):
                    ...
                    indication_drugs
                    and isinstance(indication_drugs, dict)
                    and indication_drugs.get("success")
                ):
        ...
        if len(candidate_compounds) < 3 and target_genes:
            try:
                ...
                similar_compounds = ChEMBL_search_similar_molecules(
                    query=reference_smiles, similarity_threshold=80, max_results=5
                )
                if similar_compounds:
                    # Handle both list and dict response formats
                    if isinstance(similar_compounds, list):
                        compounds_list = similar_compounds
                    elif isinstance(similar_compounds, dict) and similar_compounds.get(
                        "success"
                    ):
                    ...
                    for compound in compounds_list[:3]:
                        if isinstance(compound, dict):
                            compound_name = compound.get(
                                "molecule_chembl_id",
                                compound.get("chembl_id", "Unknown"),
                            )
                            smiles = compound.get(
        ...
        if len(candidate_compounds) == 0:
            ...
            print(
                "   💡 Consider providing target_genes or enabling more discovery strategies"
            )
        ...
        if enable_llm_analysis and candidate_compounds:
            try:
                ...
                if compound_scoring and compound_scoring.get("success"):
                    results["llm_insights"]["compound_scoring"] = compound_scoring[
                        "result"
                    ]
            ...
            try:
                    ...
                    try:
                        ...
                        if cid_data and cid_data.get("success") and cid_data["result"]:
                            ...
                            if cid:
                                ...
                                if (
                                    props_data
                                    and props_data.get("success")
                                    and props_data["result"]
                                ):
                                    smiles = props_data["result"][0].get("smiles")
                    except Exception as e:
                        print(
                            f"     ⚠️ Could not get SMILES for {compound_name}: {str(e)}"
                        )
                ...
                if smiles:
                    ...
                    try:
                        ...
                        print(
                            f"     ⚠️ Physicochemical properties prediction failed: {str(e)}"
                        )
                    ...
                    print(
                        f"     ⚠️ No SMILES available for {compound_name}, skipping ADMET prediction"
                    )
        ...
        for compound_name, admet_data in admet_profiles.items():
            try:
                ...
                if isinstance(bioavailability_data, dict):
                    bioavailability_value = bioavailability_data.get(
                        "bioavailability", 0.5
                    )
                ...
                pk_models[compound_name] = {
                    ...
                    "predicted_dose": dose,
                    "feasibility": "A"
                    if bioavailability_value > 0.3 and half_life < 24
                    else "B",
                }

                print(
                    f"   📊 PK model for {compound_name}: dose={dose:.1f}mg, t1/2={half_life:.1f}h"
                )
        ...
        for compound in candidate_compounds[:3]:  # Top 3 compounds
            ...
            try:
                ...
                if interactions:
                    ...
                    print(
                        f"   ✅ Found {len(interactions)} interactions for {compound_name}"
                    )
                else:
        ...
        for query in literature_queries:
            try:
                ...
                # Semantic Scholar
                try:
                    semantic_articles = SemanticScholar_search_papers(
                        query=query, limit=2
                    )
        ...
        try:
            ...
            if clinical_trials and clinical_trials.get("success"):
                results["preclinical_validation"]["clinical_trials"] = clinical_trials[
                    "result"
                ]
        ...
        if enable_llm_analysis and literature_results:
            try:
                ...
                if literature_synthesis and literature_synthesis.get("success"):
                    results["llm_insights"]["literature_synthesis"] = (
                        literature_synthesis["result"]
                    )
        ...
        if enable_llm_analysis and candidate_compounds:
            try:
                ...
                if lead_optimization and lead_optimization.get("success"):
                    results["llm_insights"]["lead_optimization"] = lead_optimization[
                        "result"
                    ]
        ...
        if enable_llm_analysis and candidate_compounds:
            try:
                ...
                preclinical_data = (
                    f"ADMET profiles: {len(admet_profiles)} compounds analyzed"
                )
                ...
                if clinical_design and clinical_design.get("success"):
                    results["llm_insights"]["clinical_trial_design"] = clinical_design[
                        "result"
                    ]
...
def neurodegenrative_drug_discovery(
    ...
    for mechanism in neuro_mechanisms[:6]:  # Use top 6 mechanisms
        try:
            ...
            compounds = (
                drugbank_get_drug_name_description_pharmacology_by_mechanism_of_action(
                    query=mechanism, case_sensitive=False, exact_match=False, limit=3
                )
            )
            ...
            if compounds and compounds.get("success"):
                ...
                print(
                    f"     Found {len(compounds['result'])} compounds for {mechanism}"
                )
        except Exception as e:
        ...
        try:
            ...
            if papers and isinstance(papers, list):
                ...
                for paper in papers:
                    ...
                    potential_drugs = re.findall(
                        r"\b[A-Z][a-z]+(?:[a-z]+)*\b", abstract + " " + title
                    )
    ...
    for candidate in repurposing_candidates:
        try:
            drug_info = drugbank_get_drug_name_and_description_by_target_name(
                query=candidate, case_sensitive=False, exact_match=True, limit=1
            )
    ...
    for product in natural_products:
        try:
            ...
            product_info = drugbank_get_drug_name_and_description_by_target_name(
                query=product, case_sensitive=False, exact_match=True, limit=1
            )
...
def display_unified_results_summary(results: Dict[str, Any]):
    ...
    print(
        f"   Targets identified: {len(results.get('target_identification', {}).get('validated_targets', []))}"
    )
    print(
        f"   Compounds found: {len(results.get('compound_screening', {}).get('candidate_compounds', []))}"
    )
    print(
        f"   ADMET profiles: {len(results.get('admet_assessment', {}).get('admet_profiles', {}))}"
    )
    print(
        f"   PK models: {len(results.get('pharmacokinetics', {}).get('pk_models', {}))}"
    )
    print(f"   PD data: {len(results.get('pharmacodynamics', {}).get('pd_data', {}))}")
    print(
        f"   DDI data: {len(results.get('drug_interactions', {}).get('ddi_data', {}))}"
    )
    print(
        f"   Literature: {len(results.get('preclinical_validation', {}).get('literature_results', []))}"
    )
...
if __name__ == "__main__":
    ...
    print("\n💊 Compound Discovery:")
    print(
        f"   Total compounds found: {results3['compound_screening']['total_compounds']}"
    )
