    ...
    result = tu.run(
        {
            ...
            "arguments": {"query": "cancer", "organism": "Homo sapiens", "limit": 10},
        }
    )
    ...
    if result.get("status") == "success":
        ...
        if result_data:
            ...
            for i, sample in enumerate(samples[:3]):  # Show first 3
                print(
                    f"  Sample {i + 1}: {sample.get('sample_id')} - {sample.get('characteristics')}"
                )
        else:
...
def example_combined_search():
    ...
    if search_result.get("status") == "success":
        ...
        if idlist:
            ...
            info_result = tu.run(
                {"name": "geo_get_dataset_info", "arguments": {"dataset_id": idlist[0]}}
            )
