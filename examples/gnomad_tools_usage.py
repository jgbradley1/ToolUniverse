...
def example_get_gene_constraints():
    ...
    result = tu.run(
        {"name": "gnomad_get_gene_constraints", "arguments": {"gene_symbol": "BRCA1"}}
    )
...
def example_get_gene_constraints_tp53():
    ...
    result = tu.run(
        {"name": "gnomad_get_gene_constraints", "arguments": {"gene_symbol": "TP53"}}
    )
...
def example_combined_analysis():
    ...
    for gene_symbol in genes:
        ...
        if constraints_result.get("status") == "success":
            ...
            print(
                f"Error getting constraints for {gene_symbol}: {constraints_result.get('error')}"
            )
