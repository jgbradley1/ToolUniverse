    ...
    config={
        ...
        "parameter": {
            ...
            "properties": {
                ...
                "filename": {"type": "string", "description": "CSV file path"},
                "data": {
                    "type": "array",
    ...
    def run(self, arguments: Dict[str, Any], **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            if operation == "read":
                ...
                if not data:
                    return {"result": {"error": "No data found"}, "success": False}
                ...
                for row in data:
                    if transform_column in row:
                        if transform_operation == "uppercase":
                            ...
                            row[transform_column] = transform_value + str(
                                row[transform_column]
                            )
                        elif transform_operation == "add_suffix":
                            row[transform_column] = (
                                str(row[transform_column]) + transform_value
                            )
    ...
    config={
        ...
        "parameter": {
            ...
            "properties": {
                ...
                "filename": {"type": "string", "description": "JSON file path"},
                "data": {"description": "Data to write (for write operation)"},
                "extract_path": {
                    "type": "string",
                    "description": "JSON path to extract (e.g., 'users[0].name')",
                },
                "merge_data": {"description": "Data to merge (for merge operation)"},
            },
            "required": ["operation", "filename"],
        },
    },
...
def test_csv_processing(tu):
    ...
    try:
        ...
        result = tu.run(
            {
                "name": "csv_processor_tool",
                "arguments": {"operation": "analyze", "filename": test_filename},
            }
        )
...
def test_json_processing(tu):
    ...
    try:
        ...
        result = tu.run(
            {
                "name": "json_processor_tool",
                "arguments": {"operation": "validate", "filename": test_filename},
            }
        )
...
def test_direct_access(tu):
    ...
    try:
        ...
        result = tu.tools.csv_processor_tool(
            operation="analyze", filename="nonexistent.csv"
        )
        print(f"Direct csv_processor_tool(): {result}")

        result = tu.tools.json_processor_tool(
            operation="validate", filename="nonexistent.json"
        )
