...
@register_tool(
    "HelloTool", config={"name": "hello_tool", "description": "Say hello to the user"}
)
...
@register_tool(
    ...
    config={
        ...
        "parameter": {
            "type": "object",
            "properties": {
                "name": {"type": "string", "description": "Person's name to greet"}
            },
            "required": ["name"],
        },
    },
...
class GreetTool(BaseTool):
    ...
    def run(self, arguments=None, **kwargs):
        ...
        return {"message": f"Hello, {name}!", "success": True, "greeted_person": name}
...
class MathTool(BaseTool):
    ...
    def run(self, arguments=None, **kwargs):
        ...
        try:
            ...
            return {"result": None, "error": "Invalid number format", "success": False}
        ...
        if operation == "add":
            ...
            if b == 0:
                return {"result": None, "error": "Division by zero", "success": False}
            result = a / b
        else:
            return {
                "result": None,
...
def test_basic_tools(tu):
    ...
    print("\n1. Testing HelloTool:")
    try:
        result = tu.run({"name": "hello_tool", "arguments": {}})
    ...
    # Test GreetTool
    print("\n2. Testing GreetTool:")
    test_cases = [{"name": "Alice"}, {"name": "Bob"}, {"name": "World"}]

    for i, test_case in enumerate(test_cases, 1):
        try:
            result = tu.run({"name": "greet_tool", "arguments": test_case})
    ...
    for i, test_case in enumerate(math_tests, 1):
        try:
            result = tu.run({"name": "math_tool", "arguments": test_case})
            print(f"   Test {i}: {result}")
