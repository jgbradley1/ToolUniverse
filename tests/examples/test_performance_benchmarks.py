...
class TestPerformanceBenchmarks(unittest.TestCase):
    ...
    def benchmark_tool_performance(self, tool_func, params, iterations=3):
        ...
        return {
            ...
            "success_rate": sum(1 for t in execution_times if t < 10)
            / len(execution_times),
        }
    ...
    def test_open_targets_performance(self):
        ...
        print(
            f"Disease search - Avg: {disease_benchmark['avg_time']:.2f}s, "
        ...
        self.assertLess(
            disease_benchmark["avg_time"], 15, "OpenTargets disease search too slow"
        )
        self.assertGreater(
            disease_benchmark["success_rate"], 0, "OpenTargets disease search failed"
        )
    ...
    def test_chembl_performance(self):
        ...
        print(
            f"Compound search - Avg: {compound_benchmark['avg_time']:.2f}s, "
        ...
        # Performance assertions
        self.assertLess(
            compound_benchmark["avg_time"], 20, "ChEMBL compound search too slow"
        )
        self.assertGreater(
            compound_benchmark["success_rate"], 0, "ChEMBL compound search failed"
        )
    ...
    def test_hpa_performance(self):
        ...
        gene_benchmark = self.benchmark_tool_performance(
            HPA_search_genes_by_query, {"query": "BRCA1", "limit": 5}, iterations=3
        )

        self.benchmark_results["hpa_genes"] = gene_benchmark

        print(
            f"Gene search - Avg: {gene_benchmark['avg_time']:.2f}s, "
    ...
    def test_literature_performance(self):
        ...
        arxiv_benchmark = self.benchmark_tool_performance(
            ArXiv_search_papers, {"query": "machine learning", "limit": 5}, iterations=3
        )

        self.benchmark_results["arxiv_search"] = arxiv_benchmark

        print(
            f"ArXiv search - Avg: {arxiv_benchmark['avg_time']:.2f}s, "
    ...
    def test_pdb_performance(self):
        ...
        sequence_benchmark = self.benchmark_tool_performance(
            get_sequence_by_pdb_id, {"pdb_id": "1CRN"}, iterations=3
        )

        self.benchmark_results["pdb_sequence"] = sequence_benchmark

        print(
            f"Sequence retrieval - Avg: {sequence_benchmark['avg_time']:.2f}s, "
        ...
        # Performance assertions
        self.assertLess(
            sequence_benchmark["avg_time"], 10, "PDB sequence retrieval too slow"
        )
        self.assertGreater(
            sequence_benchmark["success_rate"], 0, "PDB sequence retrieval failed"
        )
    ...
    def test_clinical_trial_performance(self):
        ...
        trial_benchmark = self.benchmark_tool_performance(
            search_clinical_trials, {"condition": "diabetes", "limit": 5}, iterations=3
        )

        self.benchmark_results["clinical_trials"] = trial_benchmark

        print(
            f"Trial search - Avg: {trial_benchmark['avg_time']:.2f}s, "
        ...
        # Performance assertions
        self.assertLess(
            trial_benchmark["avg_time"], 15, "Clinical trial search too slow"
        )
        self.assertGreater(
            trial_benchmark["success_rate"], 0, "Clinical trial search failed"
        )
    ...
    def test_overall_performance_summary(self):
        ...
        for tool_name, results in self.benchmark_results.items():
            print(
                f"  {tool_name}: {results['avg_time']:.2f}s avg, "
    ...
    def test_concurrent_tool_usage(self):
        ...
        def run_tool(tool_func, params, tool_name):
            ...
            try:
                ...
                end_time = time.time()
                results_queue.put(
                    {"tool": tool_name, "success": True, "time": end_time - start_time}
                )
        ...
        for tool_func, params, tool_name in concurrent_tools:
            thread = threading.Thread(
                target=run_tool, args=(tool_func, params, tool_name)
            )
        ...
        total_time = (
            max([r["time"] for r in concurrent_results]) if concurrent_results else 0
        )
        ...
        # Performance assertions
        self.assertGreater(
            len(successful_tools), 0, "No tools succeeded in concurrent execution"
        )
