...
class DataProcessorTool(BaseTool):
    ...
    def run(self, arguments: Dict[str, Any], **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            if validation_error:
                return {"result": None, "error": validation_error, "success": False}
...
class TextAnalyzerTool(BaseTool):
    ...
    def run(self, arguments: Dict[str, Any], **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            if not text:
                return {"result": {}, "error": "Text cannot be empty", "success": False}
...
def test_advanced_tools(tu):
    ...
    test_cases = [
        {"data": [1, 2, 3, 4, 5], "operation": "mean", "format_output": True},
        {"data": [10, 20, 30, 40, 50], "operation": "std", "format_output": True},
        {"data": [1, 2, 3, 4, 5, 6], "operation": "median", "format_output": True},
        {
            "data": [],  # Error case
            "operation": "mean",
            "format_output": True,
    ...
    for i, test_case in enumerate(test_cases, 1):
        try:
            result = tu.run({"name": "data_processor_tool", "arguments": test_case})
    ...
    for i, test_case in enumerate(text_tests, 1):
        try:
            result = tu.run({"name": "text_analyzer_tool", "arguments": test_case})
            print(f"   Test {i}: {result}")
    ...
    try:
        ...
        result = tu.run(
            {
                "name": "file_processor_tool",
                "arguments": {"operation": "read", "filename": test_filename},
            }
        )
        ...
        result = tu.run(
            {
                "name": "file_processor_tool",
                "arguments": {"operation": "info", "filename": test_filename},
            }
        )
...
def test_direct_access(tu):
    ...
    try:
        # Test direct access
        result = tu.tools.data_processor_tool(data=[1, 2, 3, 4, 5], operation="mean")
        print(f"Direct data_processor_tool(): {result}")

        result = tu.tools.text_analyzer_tool(
            text="Hello world!", metrics=["word_count", "char_count"]
        )
